...
import { useTambahStaf } from "../../features/instansi/useTambahStaf";

export default function TambahStafInstansi() {
    const { onAddStaf } = useTambahStaf();
    ...
    const hariPenugasanList = ["Senin", "Selasa", "Rabu", "Kamis", "Jum'at", "Sabtu", "Minggu"];
    ...
    const handleSubmitForm = e => {
        ...
        if (hariDipilih.toLowerCase() === "pilih") {
            notifikasi("Formulir gagal", "Pilih hari penugasan staf terlebih dahulu");
            return;
        }

        // jika gambar belum diinput
        if (!foto?.name) {
            notifikasi("Formulir gagal", "Unggah file atau gambar ke dalam formulir terlebih dahulu", "error");
            return;
        }

        // jika ukuran foto melebihi 2mb
        if (sizeFoto > 2) {
            notifikasi("Formulir gagal!", "Ukuran file/gambar yang diunggah terlalu besar", "error");
            return;
        }
        const data = {
            nama_staf: nama,
        ...
        // kirim data ke BE
        const item_token = JSON.parse(localStorage.getItem("login-instansi"));
        onAddStaf({ data:formData, token: item_token.token });
    };

    return (
        <form onSubmit={e => handleSubmitForm(e)} className="p-4 dark:text-white">
            {/* header dan tombol submit */}
            <div className="flex items-center justify-between">
                <span className="font-bold text-lg sm:text-xl">Tambah Staf</span>
            ...
            <div className="mt-8 shadow-md/10 gap-10 rounded-md py-8 px-5 sm:px-10 grid grid-cols-1 sm:grid-cols-2 mx-0 sm:mx-5">
                ...
                <div>
                    ...
                    <div className="mb-7 flex flex-col gap-2.5">
                        <label htmlFor="nama-lengkap-instansi" className="text-sm sm:text-base">
                            Nama Lengkap
                        </label>
                        <input
                    ...
                    <div className="mb-7 flex flex-col gap-2.5">
                        <label htmlFor="nomor-induk-pegawai-instansi" className="text-sm sm:text-base">
                            Nomor Induk Pegawai
                        </label>
                        <input
                    ...
                    <div className="mb-7 flex flex-col gap-2.5">
                        <label htmlFor="kontak" className="text-sm sm:text-base">
                            Kontak
                        </label>
                        <input
                            id="kontak"
                    ...
                    <div className="flex flex-col gap-2.5">
                        <span className="text-sm sm:text-base">Hari Penugasan</span>
                        <div className="dropdown-box relative">
                            <button id="pilih" type="button" onClick={() => setMunculkanDropdown(!munculkanDropdown)} className={dropdownStyle}>
                                <span className="text-sm sm:text-base">{hariDipilih}</span>
                                {munculkanDropdown ? <MdKeyboardArrowUp /> : <MdKeyboardArrowDown />}
                            </button>
                            ...
                            <div
                                ...
                                style={{
                                    display: munculkanDropdown ? "block" : "none",
                                }}
                            >
                ...
                <div {...getRootProps()} className="border-none outline-none">
                    ...
                    <div
                        className={`
                            ${sedangDiDrag ? "bg-[#BCF0DA]" : "bg-[#E5E7EB] dark:bg-gray-700"}
                    ...
                    >
                        ...
                        {foto?.name ? (
                            <img src={urlFoto} alt="preview image" className="w-125 h-54 sm:h-62 object-cover rounded-md" />
                        ) : (
                    ...
                    <div className="text-xs my-1">
                        {sizeFoto > 2 ? (
                            <span className="text-red-500">*Ukuran foto terlalu besar ({sizeFoto}mb)</span>
                        ) : (
                            <span className="text-green-500">{foto?.name && <>*Memenuhi standar ({sizeFoto}mb)</>}</span>
                        )}
                    </div>
                    ...
                    <div className="mt-3">
                        ...
                        <label htmlFor="input-foto-instansi">
                            <div className="flex flex-nowrap border border-gray-200 dark:border-gray-700 rounded-md">
                                <span
                                    className={`
                                        ${sedangDiDrag || foto?.name ? "bg-[#BCF0DA]" : "bg-[#E5E7EB] dark:bg-gray-600"}
                                ...
                                <span className="flex items-center line-clamp-1 ms-2">
                                    {foto?.name && <span className="line-clamp-1 text-sm sm:text-base">{foto?.name}</span>}
                                </span>
                            </div>
                        </label>
                        <input {...getInputProps()} id="input-foto-instansi" type="file" name="file-gambar-instansi" className="hidden" />
                        <p className="mt-1 italic text-[.6rem] text-red-600">
                            Dokumen yang diunggah harus berupa gambar dan menggunakan ekstensi .jpg, .jpeg, .png, .bmp / maksimal ukuran file 2mb
                        </p>
                    </div>
                </div>
            </div>
