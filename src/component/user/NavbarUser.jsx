...
export default function NavbarUser({ setIsOpen,isOpen }) {
        ...
        return (
                <Navbar fluid className="p-4 w-full sticky top-0 z-20 dark:bg-gray-800">
                        ...
                        <div className="flex items-center gap-2">
                                <Button color="transparent" className="flex items-center bg-gray-900 text-gray-50 dark:text-black dark:hover:bg-gray-100 hover:bg-gray-700 dark:bg-white gap-1 h-6 xs:h-[30px] px-2 xs:px-3 text-[.5rem] xs:text-[.6rem] sm:text-xs sm:px-5 sm:h-10" onClick={() => navigate("/user/buat-laporan")}>
                                        <i className="fas fa-plus"></i>
                                        Buat Laporan
                                </Button>

                                {/* bagian set theme nya */}
                                <span
                                        onClick={() => setDarkThemeUser(!darkThemeUser)}
                                        className="
                                                dark:bg-sky-300 bg-amber-300
                                                cursor-pointer h-7 w-7 xs:h-[30px] xs:w-[30px] sm:h-9 sm:w-9 flex text-white
                                                items-center justify-center rounded-[50%]
                                ">
                                        <i className={`text-xs sm:text-base ${darkThemeUser ? "fas fa-moon" : "fas fa-sun"}`}></i>
                                </span>
                        </div>
                </Navbar>
        );
